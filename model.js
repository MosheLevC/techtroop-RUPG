...
const generateBaconIpsum = () =>
  axios
    .get("https://baconipsum.com/api/?type=meat-and-filler&sentences=2&format=text")
    .then(({ data }) => {
...
export const saveUserInfo = (data) => {
  ...
  const users = JSON.parse(localStorage.getItem("rupgUsers")) || {};
  const name = `${data.userInfo.fNAme} ${data.userInfo.lName}`;
  users[name] = data;
  localStorage.setItem("rupgUsers", JSON.stringify(users));
  return name;
};

export const loadUserInfo = (name) => {
  const users = JSON.parse(localStorage.getItem("rupgUsers")) || {};
  return users[name] || null;
};

export const getSavedUsersList = () => {
  const users = JSON.parse(localStorage.getItem("rupgUsers")) || {};
